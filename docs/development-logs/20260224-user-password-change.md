...
### 问题1：Docker镜像内代码难以修改
- **问题描述：** 在 Docker 容器镜像内修改代码需要每次复制文件到容器，开发不便
- **解决方案：**
  - 创建 `docker-compose-dev.yml` 开发环境配置
  - 使用 Volume 挂载本地代码目录到容器
  - 本地修改代码即时生效，无需复制
  - 详细文档：`docs/DEVELOPMENT_ENVIRONMENT_SETUP.md`

### 问题2：createToast 导入错误
- **问题描述：** 前端构建失败，`createToast` 不是 frappe-ui 的导出项
- **错误信息：** `"createToast" is not exported by "node_modules/frappe-ui/src/index.ts"`
- **解决方案：**
  - 查看其他组件的用法
  - 将 `import { createToast }` 改为 `import { toast }`
  - 使用 `toast.success()` 替代 `createToast()`
  - 与项目其他组件保持一致
...
- **目标分支：** custom-dev
- **提交数量：** 3 个提交
  - `c1cec008` feat(user): 添加用户密码修改功能
  - `3f35f90f` fix(user): 修复密码修改组件的toast导入错误
  - `056372f9` docs: 将开发环境配置规则添加到二开规范
- **版本号：** （待合并到 main 时确定）
- **发布说明：**
  - 新增用户密码自主修改功能，提升系统安全性
  - 新增开发环境配置，提升二次开发效率
  - 完善二次开发规范文档
