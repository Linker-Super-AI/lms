...
"""
直接执行脚本来创建 LMS 课程
使用方法: bench --site 192.168.20.118 execute lms.create_course_script.main
"""
...
def main():
    """主函数"""
    # 读取结构文件
    structure_file = "/tmp/course_structure.json"
    ...
    for chapter_idx, chapter_data in enumerate(chapters, 1):
        ...
        if sections:
            ...
            for lesson_idx, section in enumerate(sections, 1):
                lesson = create_lesson(section, chapter.name, course.name, lesson_idx)
                print(f"    [{lesson_idx}] ✓ {section['title']}")
                lesson_count += 1
        else:
            lesson = create_default_lesson(chapter_title, chapter.name, course.name)
            print(f"  ✓ 创建默认课时")
            lesson_count += 1

        # 创建测验
        print(f"  创建章节测验:")
        create_quiz_lesson(chapter_title, chapter.name, course.name, lesson_count + 1)
        print(f"  ✓ 测验课时已添加\n")
...
def create_lesson(section, chapter_name, course_name, idx):
    """创建课时"""
    ...
    lesson.update({
        ...
        "course": course_name,
        "content": content,
        "include_in_preview": 1 if idx == 1 else 0,
    })
...
def create_default_lesson(chapter_title, chapter_name, course_name):
    """创建默认课时"""
    lesson = frappe.new_doc("Course Lesson")
    lesson.update({
...
def create_quiz_lesson(chapter_title, chapter_name, course_name, idx):
    """为章节创建测验"""
    # 创建问题
    questions = []
    for i in range(1, 4):  # 3个问题
